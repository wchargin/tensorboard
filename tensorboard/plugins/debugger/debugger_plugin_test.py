...
class DebuggerPluginTest(debugger_plugin_testlib.DebuggerPluginTestBase):
    def testHealthPillsRouteProvided(self):
        """Tests that the plugin offers the route for requesting health
        pills."""
        ...
        response = self.server.post(
            ...
            data={
                "node_names": json.dumps(
                    ["layers/Variable", "unavailable_node"]
                ),
                "run": "run_foo",
            },
        )
                ...
                "layers/Variable": [
                    {
                        ...
                        "shape": [8.0],
                        "value": list(range(12))
                        + [tf.int16.as_datatype_enum, 1.0, 8.0],
                    }
                ],
            },
    ...
    def testRequestHealthPillsForDefaultRun(self):
        """Tests that the plugin produces health pills for the default '.'
        run."""
    ...
    def testRequestsWithoutProperPostKeyUnsupported(self):
        """Tests that requests lacking the node_names POST key are
        unsupported."""
    ...
    def testRequestsWithBadJsonUnsupported(self):
        ...
        response = self.server.post(
            "/data/plugin/debugger/health_pills",
            data={"node_names": "some obviously non JSON text",},
        )
        self.assertEqual(400, response.status_code)

    def testRequestsWithNonListPostDataUnsupported(self):
        """Tests that requests with loads lacking lists of ops are
        unsupported."""
        response = self.server.post(
            "/data/plugin/debugger/health_pills",
            data={
                "node_names": json.dumps({"this is a dict": "and not a list."}),
            },
        )
        self.assertEqual(400, response.status_code)
                ...
                "layers/Matmul": [
                    {
                        ...
                        "value": (
                            list(range(12))
                            + [
                                float(tf.float64.as_datatype_enum),
                                2.0,
                                3.0,
                                3.0,
                            ]
    ...
    def testNoHealthPillsForSpecificStep(self):
        """Tests that an empty mapping is returned for no health pills at a
        step."""
        response = self.server.post(
            "/data/plugin/debugger/health_pills",
            data={
    ...
    def testDebuggerDataServerNotStartedWhenListenIsNotCalled(self):
        """Tests that the plugin starts no debugger data server if port is
        None."""
        self.mock_debugger_data_server_class.reset_mock()

        # Initialize a debugger plugin with no GRPC port provided.
        self.debugger_plugin_module.DebuggerPlugin(
            self.context
        ).get_plugin_apps()
