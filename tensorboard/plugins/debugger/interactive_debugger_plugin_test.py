...
This test launches an instance InteractiveDebuggerPlugin as a separate
thread. The test then calls Session.run() using RunOptions pointing to
the grpc:// debug URL of the debugger data server. It then sends HTTP
requests to the TensorBoard backend endpoints to query and control the
state of the Sessoin.run().
"""
...
class InteractiveDebuggerPluginTest(tf.test.TestCase):
    def setUp(self):
        ...
        context = base_plugin.TBContext(
            logdir=self._dummy_logdir, multiplexer=dummy_multiplexer
        )
    ...
    def testCommAndAckWithoutBreakpoints(self):
        (
            session_run_thread,
            session_run_results,
        ) = self._runSimpleAddMultiplyGraph()
    ...
    def testRetrieveAllGatedGrpcTensors(self):
        ...
        tensor_names = [
            item[0]
            for item in retrieve_all_data["gated_grpc_tensors"][device_name]
        ]
        self.assertItemsEqual(
            ["a", "a/read", "b", "b/read", "x", "c", "c/read", "y"],
            tensor_names,
        )
    ...
    def testInvalidBreakpointStateLeadsTo400Response(self):
        ...
        self.assertEqual(
            "Unrecognized new state for x:0:DebugIdentity: bad_state",
            data["error"],
        )
    ...
    def testInvalidModeArgForGatedGrpcRouteLeadsTo400Response(self):
        ...
        data = self._deserializeResponse(response)
        self.assertEqual(
            "Unrecognized mode for the gated_grpc route: bad_mode",
            data["error"],
        )
    ...
    def testGetSourceFilePaths(self):
        ...
        source_paths_response = self._serverGet(
            "source_code", {"mode": "paths"}
        )
    ...
    def testHealthPill(self):
        ...
        self.assertAllClose(
            np.var([10.0, 20.0, 30.0]), tensor_data[11]
        )  # Variance.
    ...
    def testBinaryStringTensorIsHandledCorrectly(self):
        ...
        self.assertAllEqual(
            np.array(
                [b"\x01\x01\x01\x03\x03\x03", b"\x02\x02\x02\x04\x04\x04"],
                dtype=np.object,
            ),
