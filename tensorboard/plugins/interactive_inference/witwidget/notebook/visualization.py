...
class WitConfigBuilder(object):
    ...
    def build(self):
        ...
        Used by WitWidget to set the settings on an instance of the
        What-If Tool.
        """
        return self.config
    ...
    def set_examples(self, examples, feature_names=None):
        ...
        if len(examples) > 0 and not (
            isinstance(examples[0], tf.train.Example)
            or isinstance(examples[0], tf.train.SequenceExample)
        ):
    ...
    def set_model_version(self, version):
        """Sets the optional model version for model inference through TF
        Serving.
    ...
    def set_model_signature(self, signature):
        """Sets the optional model signature for model inference through TF
        Serving.
    ...
    def set_compare_inference_address(self, address):
        """Sets the inference address for model inference for a second model
        hosted by TF Serving.
    ...
    def set_compare_model_version(self, version):
        """Sets the optional model version for a second model hosted by TF
        Serving.
    ...
    def set_predict_input_tensor(self, tensor):
        """Sets the name of the input tensor for models that use the Predict
        API.
    ...
    def set_predict_output_tensor(self, tensor):
        """Sets the name of the output tensor for models that need output
        parsing.
