...
class InferenceUtilsTest(tf.test.TestCase):
    ...
    def test_example_protos_from_path_get_all_in_file(self):
        cns_path = os.path.join(
            tf.compat.v1.test.get_temp_dir(), "dummy_example"
        )
        example = test_utils.make_fake_example()
    ...
    def test_example_proto_from_path_if_does_not_exist(self):
        cns_path = os.path.join(
            tf.compat.v1.test.get_temp_dir(), "does_not_exist"
        )
    ...
    def test_get_numeric_features_to_observed_range(self):
        ...
        data = inference_utils.get_numeric_features_to_observed_range([example])

        # Returns a sorted list by feature_name.
        self.assertDictEqual(
            {
                "repeated_float": {"observedMin": 1.0, "observedMax": 4.0,},
                "repeated_int": {"observedMin": 10, "observedMax": 20,},
                "single_float": {"observedMin": 24.5, "observedMax": 24.5,},
                "single_int": {"observedMin": 2.0, "observedMax": 2.0,},
            },
            data,
        )
    ...
    def test_get_categorical_features_to_sampling(self):
        ...
        self.assertDictEqual({"non_numeric": {"samples": ["cat"]}}, data)
        ...
        self.assertDictEqual(
            {"non_numeric": {"samples": ["pony", "cow"]}}, data
        )
    ...
    def test_make_mutant_tuples_float_list(self):
        ...
        np.testing.assert_almost_equal(
            expected_values,
            [
                mutant_example.features.feature[
                    "repeated_float"
                ].float_list.value[index_to_mutate]
                for mutant_example in mutant_examples
            ],
        )

        # Check that the example (other than the mutant value) is the same.
        for expected_value, mutant_example in zip(
            expected_values, mutant_examples
        ):
    ...
    def test_make_mutant_tuples_int_list(self):
        ...
        np.testing.assert_almost_equal(
            ...
            [
                mutant_example.features.feature[
                    "repeated_int"
                ].int64_list.value[index_to_mutate]
                for mutant_example in mutant_examples
            ],
        )
    ...
    def test_convert_predict_response_classification(self):
        ...
        dim = output.tensor_shape.dim.add()
        dim.size = 2
        output.float_val.extend([1.0, 0.0, 0.9, 0.1, 0.8, 0.2])
        ...
        self.assertEqual(
            "0", converted.result.classifications[0].classes[0].label
        )
        self.assertAlmostEqual(
            1, converted.result.classifications[0].classes[0].score
        )
        self.assertEqual(
            "1", converted.result.classifications[0].classes[1].label
        )
        self.assertAlmostEqual(
        ...
        self.assertEqual(
            "0", converted.result.classifications[1].classes[0].label
        )
        self.assertAlmostEqual(
            0.9, converted.result.classifications[1].classes[0].score
        )
        self.assertEqual(
            "1", converted.result.classifications[1].classes[1].label
        )
        self.assertAlmostEqual(
            0.1, converted.result.classifications[1].classes[1].score
        )

        self.assertEqual(
            "0", converted.result.classifications[2].classes[0].label
        )
        self.assertAlmostEqual(
            0.8, converted.result.classifications[2].classes[0].score
        )
        self.assertEqual(
            "1", converted.result.classifications[2].classes[1].label
        )
        self.assertAlmostEqual(
            0.2, converted.result.classifications[2].classes[1].score
        )
