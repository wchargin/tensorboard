...
def get_worker_list(cluster_resolver):
    ...
    worker_list = [
        cluster_spec.task_address("worker", i).split(":")[0]
...
class ProfilePlugin(base_plugin.TBPlugin):
    ...
    def __init__(self, context):
        ...
        self.master_tpu_unsecure_channel = (
            context.flags.master_tpu_unsecure_channel
        )
        ...
        if not self._is_active and self._is_active_lock.acquire(False):
            if self._is_active:
                ...
                new_thread = threading.Thread(
                    target=compute_is_active, name="ProfilePluginIsActiveThread"
                )
                new_thread.start()
    ...
    def frontend_metadata(self):
        ...
        return base_plugin.FrontendMetadata(
            element_name="tf-profile-dashboard", disable_reload=True,
        )
    ...
    def start_grpc_stub_if_necessary(self):
        ...
        if self.master_tpu_unsecure_channel and self.logdir.startswith("gs://"):
            if self.stub is None:
                ...
                options = [
                    ("grpc.max_message_length", gigabyte),
                ...
                self.stub = profiler_analysis_pb2_grpc.ProfileAnalysisStub(
                    channel
                )
    ...
    def _get_active_tools(self, profile_run_dir):
        ...
        if "trace_viewer@" in tools:
            ...
            removed_tool = (
                "trace_viewer@" if self.stub is None else "trace_viewer"
            )
    ...
    def host_impl(self, run, tool):
        ...
        try:
            ...
            hosts = [
                os.path.basename(f).replace(TOOLS[tool], "") for f in files
            ]
        except tf.errors.OpError as e:
            logger.warn(
    ...
    def capture_route(self, request):
        ...
        if is_tpu_name:
            ...
            master_ip = master_grpc_addr.replace("grpc://", "").replace(
                ":8470", ""
            )
