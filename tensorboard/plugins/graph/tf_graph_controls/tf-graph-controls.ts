...
// TODO(stephanwlee): Move this to tf-graph-dashboard
export interface Dataset extends Array<RunItem> {
}
...
Polymer({
  ...
  _traceInputToggleChanged: function(event: Event): void {
    ...
    // Needs to be casted to PaperToggleButton but constructor is missing.
    const PaperToggleButton: any = event.target;
    this.renderHierarchy.traceInputs = PaperToggleButton.active;
  ...
  _getCurrentDevices: function(
    ...
    const all_devices = devStats.map((d) => d.device);
    const devices = all_devices.filter((deviceName) => {
  ...
  _getTags: function(
      datasets: Dataset, _selectedRunIndex: number): TagItem[] {
  ...
  _isGradientColoring: function(
    ...
    return GRADIENT_COMPATIBLE_COLOR_BY.has(colorBy) &&
        this._statsNotNull(stats);
  },
  ...
  _getCurrentDeviceParams: function(
      colorByParams: ColorByParams): DeviceColor[] {
  ...
  _getCurrentXlaClusterParams: function(
      colorByParams: ColorByParams): XlaClusterColor[] {
    return colorByParams.xla_cluster;
  },

  _getCurrentGradientParams: function(
      colorByParams: ColorByParams, colorBy: ColorBy): ColorParams|void {
  ...
  _getSelectionOpGraphDisabled(datasets: Dataset,
      _selectedRunIndex: number, _selectedTagIndex: number) {
  ...
  _getSelectionProfileDisabled(datasets: Dataset,
      _selectedRunIndex: number, _selectedTagIndex: number) {
  ...
  _getSelectionConceptualGraphDisabled(datasets: Dataset,
      _selectedRunIndex: number, _selectedTagIndex: number) {
